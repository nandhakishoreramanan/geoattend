...
(function (global) {
  const Organizer = {
    ...
    bindEvents() {
      ...
      // AI Generate Description in Create Event Form (Bonus ⭐)
      const btnAiGenDesc = document.getElementById('btnAiGenDesc');
      if (btnAiGenDesc) {
        btnAiGenDesc.addEventListener('click', () => this.autoGenerateDescription());
      }
      ...
      // Refresh AI Insights Button (Bonus ⭐)
      const btnRefreshAi = document.getElementById('btnRefreshAiInsights');
      if (btnRefreshAi) {
        btnRefreshAi.addEventListener('click', () => this.loadAiInsights(this.currentEventId));
      }
    ...
    async loadEvents() {
      try {
        ...
        if (this.events.length > 0) {
          const targetId = this.currentEventId || this.events[0].id;
    ...
    async loadAiInsights(eventId) {
      try {
        ...
        console.warn('AI insights load note:', err);
      }
    },
    ...
    renderAiInsights(insights) {
      ...
      const healthEl = document.getElementById('aiHealthScore');
      const turnoutEl = document.getElementById('aiProjectedTurnout');
      const summaryEl = document.getElementById('aiExecutiveSummary');
      const recsList = document.getElementById('aiRecommendationsList');
      const punctualityEl = document.getElementById('aiPunctualityRate');

      if (healthEl) healthEl.textContent = `${insights.healthScore}/100`;
      if (turnoutEl) turnoutEl.textContent = insights.projectedTurnout;
      if (punctualityEl) punctualityEl.textContent = `${insights.punctualityRate}%`;
      if (summaryEl) summaryEl.textContent = insights.executiveSummary;

      if (recsList && insights.recommendations) {
        recsList.innerHTML = '';
        insights.recommendations.forEach(r => {
          const li = document.createElement('li');
          li.className = 'flex items-start gap-2 text-xs text-slate-300';
          li.innerHTML = `<span class="text-blue-400 mt-0.5 font-bold">›</span><span>${r}</span>`;
          recsList.appendChild(li);
        });
      }

      // Render anomalies if present
      const anomalyContainer = document.getElementById('aiAnomaliesContainer');
      if (anomalyContainer) {
        if (insights.anomalies && insights.anomalies.length > 0) {
          anomalyContainer.classList.remove('hidden');
          anomalyContainer.innerHTML = insights.anomalies.map(a => `
            <div class="p-2.5 rounded-lg bg-rose-500/10 border border-rose-500/30 text-xs text-rose-300 flex items-center gap-2">
              <span class="font-bold">⚠️ ${a.severity}:</span> ${a.message}
            </div>
          `).join('');
        } else {
    ...
    async autoGenerateDescription() {
      ...
      if (!title) {
        window.App?.showToast('Please enter an Event Title first', 'warning');
        return;
      }

      try {
        window.App?.showToast('AI generating professional description & agenda...', 'info');
        const res = await fetch('/api/ai/generate-description', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ title, venue, category: 'academic' })
        });
        const data = await res.json();
        ...
        if (radiusInput && data.suggestedRadius) {
          radiusInput.value = data.suggestedRadius;
          if (radiusVal) radiusVal.textContent = `${data.suggestedRadius}m`;
        }
        window.App?.showToast('AI description generated!', 'success');
      } catch (err) {
        window.App?.showToast('Error generating AI description', 'error');
      }
    },
